...
<!-- (DONE) All of the dog data is stored in the `db.json` file. You'll want to access this data
...
### STEP 2: ADD PUPS TO DOG BAR
 <!-- (DONE) On the page, there is a `div` with the id of `"dog-bar"`. On page load, make a fetch
...
<!-- (DONE) When a user clicks on a pup's `span` in the dog bar, that pup's info (`image`, `name`, and `isGoodDog` status) should show up in the `div` with the id of `"dog-info"`.
When you have the pup's information, the dog info `div` should have the following children:
 ...
 <!-- ### STEP 4: TOGGLE GOOD DOG
  (DONE) When a user clicks the Good Dog/Bad Dog button, two things should happen:
